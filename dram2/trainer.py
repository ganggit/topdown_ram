...
class Trainer:
    ...
    def __init__(self, config, data_loader):
        ...
        self.is_covnet = config.is_covnet
        # reinforce params
        self.std = config.std
        self.M = config.M
        ...
        if config.is_train:
            ...
            if hasattr(self.train_loader.sampler, 'indices'):
        ...
        '''
        self.model_name = "ram_{}_{}x{}_{}_{}layers_{}".format(
            config.num_glimpses,
            config.patch_size,
            config.patch_size,
            config.glimpse_scale, config.num_layers, config.is_covnet
        )
        ...
        self.model = RecurrentAttention(
            ...
            self.hidden_size,
            self.num_classes,
            self.is_lstm, self.num_layers, self.is_covnet
        )
        self.model.to(self.device)
        
        self.average_model = copy.deepcopy(self.model)
            ...
            self.save_checkpoint(
                {
                    ...
                    "average_model": self.average_model.state_dict(),
    ...
    def train_one_epoch(self, epoch):
        ...
        with tqdm(total=self.num_train) as pbar:
            for i, (x, y) in enumerate(self.train_loader):
                ...
                yp = torch.empty(y.shape[0], y.shape[1]+1).fill_(10)
                yp[:, :-1] = y
                y = yp.type(torch.int64)
                ...
                self.batch_size, c, im_high, im_wide = x.shape
                step = (int)(self.num_glimpses/y.shape[1])
                h_t, l_t = self.reset()
                if self.is_lstm:
                    ...
                    if(self.num_layers==1):
                        state_h[0] = self.model.cnet(x)
                    else:
                        state_h[-1] = self.model.cnet(x)
                    '''
                    state_h[-1] = self.model.contextNN(x)
                    _, l_t = self.model.locator(state_h[-1].to(self.device))
                    state_c = torch.zeros(self.num_layers, self.batch_size, self.hidden_size)
                    h_t = []
                    for i in range(self.num_layers):
                        h_t.append((state_h[i].unsqueeze(0).repeat(2,1,1).to(self.device), state_c[i].unsqueeze(0).repeat(2,1,1).to(self.device)) )
                ...
                for midx in range(mask.shape[0]):
                    ...
                    if len(maskindex)>0:
                        mask[midx, maskindex[0]:] = False 

                gmask =  torch.ones([y.shape[0], self.num_glimpses], dtype=torch.bool, device=self.device) 
                for it in range(R.shape[1]):
                    gmask[:, step*it: step*(it+1)]  = mask[:, it].unsqueeze(1).repeat(1, step)    
                    # gmask[:, 3*(it+1)-1]  = mask[:, it] 
                
                Rewards = torch.zeros([y.shape[0], self.num_glimpses], dtype=torch.float32)
                for it in range(R.shape[1]):
                    Rewards[:, step*(it+1) - 1]  = R[:, it] * (mask[:, it].float()) #mask out
                
                for it in range(1, self.num_glimpses): # discount info
                    revit = self.num_glimpses - it
                    Rewards[:, revit - 1] = Rewards[:, revit-1] + self.discount * Rewards[:, revit]
                    # log_pi[:, revit - 1] =   log_pi[:, revit - 1]  + log_pi[:, revit] *(gmask[:, revit].float())
                # Rewards = Rewards/y.shape[1]
                R = Rewards.to(self.device) 
                
                # R = Rewards[:, -1].to(self.device).unsqueeze(1).repeat(1, self.num_glimpses)                     
                

                for it in range(0, self.num_glimpses-1): # no discount info
                ...
                Rewards = (torch.sum(R, 1)==R.shape[1]).float() # torch.zeros(R.shape, dtype=torch.float32, requires_grad=False)
                Rewards = Rewards[:, None].repeat(1, R.shape[1])
                # Rewards = R
                scale = 100*(1-R).to(self.device) 
                # Rewards = Rewards.to(self.device) 
                for it in range(1, Rewards.shape[1]): # discount info
                    revit = Rewards.shape[1] - it
                    Rewards[:, revit - 1] = Rewards[:, revit-1] + self.discount * Rewards[:, revit] #*(mask[:, revit].float())
                
                loss_baseline = F.mse_loss(baselines, Rewards.detach())
                # loss_baseline = F.mse_loss(baselines*gmask, gmask*R.detach())
                # compute reinforce loss
                # summed over timesteps and averaged across batch
                adjusted_reward = Rewards - baselines.detach()# [:, 0].unsqueeze(1).repeat(1, self.num_glimpses).detach()
                # loss_reinforce = torch.sum(-log_pi * adjusted_reward*(gmask.float()), dim=1)
                loss_reinforce = torch.sum(-log_pi * adjusted_reward, dim=1)
                loss_reinforce = torch.mean(loss_reinforce, dim=0)

                l1_regularization, l2_regularization = torch.tensor(0.0).to(self.device), torch.tensor(0.0).to(self.device)

                for param in self.model.parameters():
                    l1_regularization += torch.norm(param, 1)**2
                    l2_regularization += torch.norm(param, 2)**2
                reg_loss = 0.00000001*(l1_regularization) 
            ...
            self.tau = 0.005
            for param, target_param in zip(self.model.parameters(), self.average_model.parameters()):
                target_param.data.copy_(self.tau * param.data + (1 - self.tau) * target_param.data)

                # break # for testing/evaluation
    ...
    def validate(self, epoch):
        ...
        for i, (x, y) in enumerate(self.valid_loader):
            ...
            y = y.repeat(self.M, 1, 1)
            y = torch.reshape(y, (self.M*y.shape[1], -1))
            # initialize location vector and hidden state
            ...
            if self.is_lstm:
                ...
                h_t = []
                for i in range(self.num_layers):
                    h_t.append((state_h[i].unsqueeze(0).to(self.device), state_c[i].unsqueeze(0).to(self.device)) )
            ...
            '''
            log_probas = torch.stack(log_pbs).transpose(1, 0)
            # average
            log_probas = torch.reshape(log_probas, (self.M, -1, log_probas.shape[-1]))
            log_probas = torch.mean(log_probas, dim=0)
            '''
            baselines = baselines.contiguous().view(self.M, -1, baselines.shape[-1])
            baselines = torch.reshape(baselines, (self.M*baselines.shape[1], baselines.shape[-1])) #torch.mean(baselines, dim=0)

            log_pi = log_pi.contiguous().view(self.M, -1, log_pi.shape[-1])
            # log_pi = torch.mean(log_pi, dim=0)
            log_pi = torch.reshape(log_pi, (self.M*log_pi.shape[1], log_pi.shape[-1]))
            ...
            val_pred = torch.reshape(val_pred, (self.M, -1))
            val_true = torch.reshape(val_true, (self.M, -1))
            
            correct = (val_pred==val_true).float()
            '''
            correct = (predicted == y).float()
            acc = torch.mean(100 * (torch.sum(correct, dim=1) / correct.shape[1] ==1).sum()/correct.shape[0])
    ...
    def test(self):
        ...
        for i, (x, y) in enumerate(self.test_loader):
            ...
            # log_pbs.append(log_probas)
            logits = torch.stack(log_pbs).transpose(1, 0)
            log_probas = logits.view(self.M, -1, logits.shape[-2], logits.shape[-1])
            log_probas = torch.mean(log_probas, dim=0)

            pred = log_probas.data.max(-1, keepdim=True)[1]
            pred = pred.squeeze(-1)
            # correct += pred.eq(y.data.view_as(pred)).cpu().sum()
            bb = torch.eq(pred, y.data.view_as(pred)).sum(1)
            correct += torch.sum(bb==6)
    ...
    def load_checkpoint(self, best=False):
        ...
        self.average_model.load_state_dict(ckpt["average_model"])
        self.optimizer.load_state_dict(ckpt["optim_state"])
        # reset parameters
        for name, module in self.model.contextNN.named_children():
            print('resetting ', name)
            # module.reset_parameters()
            # weights = module.weight # get_weights()
            #if name == 'fc' and self.config.resume==False:
