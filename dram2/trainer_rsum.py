    ...
    def train_one_epoch(self, epoch):
        ...
        with tqdm(total=self.num_train) as pbar:
            for i, (x, y) in enumerate(self.train_loader):
                ...
                for it in range(1, self.num_glimpses): # no discount info
                    revit = self.num_glimpses - it
                    Rewards[:, revit - 1] = Rewards[:, revit-1] + Rewards[:, revit]
                ...
                loss_baseline =0.0
                loss_reinforce = 0.0
                for it in range(0, self.num_glimpses, step): # no discount info
                    loss_baseline += F.mse_loss(baselines[:, it], Rewards[:, it].detach())
                    # R = Rewards[:, it].to(self.device).unsqueeze(1).repeat(1, self.num_glimpses) 
                    # compute reinforce loss
                    # adjusted_reward = R - baselines[:, it].unsqueeze(1).repeat(1, self.num_glimpses).detach()
                    tmp = -log_pi[:, it] * (Rewards[:, it]-baselines[:, it]).detach()
                    # loss_reinforce = torch.sum(-log_pi * adjusted_reward, dim=1)
                    loss_reinforce += torch.mean(tmp, dim=0)
                    
                    #loss_reinforce.backward(retain_graph=True)
                    #self.optimizer.step()
                    #self.optimizer.zero_grad()
