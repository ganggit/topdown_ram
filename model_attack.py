...
class RecurrentAttention(nn.Module):
    ...
    def __init__(
        self, g, k, s, c, h_g, h_l, std, hidden_size, num_classes, is_lstm
    ):
        """Constructor.
        ...
        # self.cnet = modules.ContextNetwork(512, hidden_size) # for 32*32
        self.cnet = modules.CNetwork(g, 2048, hidden_size)
        self.sensor = modules.GlimpseCovNet(h_g, h_l, g, k, s, c)
        if is_lstm:
            self.rnn = modules.CoreRNN(hidden_size, hidden_size) 
        else:
            self.rnn = modules.CoreNetwork(hidden_size, hidden_size)
        self.locator = modules.LocationNetwork(hidden_size, 2, std)
        self.features = VGG('VGG16', hidden_size)
        self.classifier = modules.ActionNetwork(2*hidden_size, num_classes)
        self.baseliner = modules.BaselineNetwork(hidden_size, 1)
        self.metacontrollor = modules.DQN(16)
    ...
    def forward(self, x, l_t_prev, h_t_prev, last=False):
        ...
        if self.is_lstm:
            ...
            log_pi, l_t = self.locator(output.squeeze(0))
            b_t = self.baseliner(output.squeeze(0)).squeeze()
        else:
        ...
        if last:
            if self.is_lstm:
                ...
                log_probas = self.classifier(torch.cat((features, output.squeeze(0)), 1))
            else:
