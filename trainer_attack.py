...
from model_attack import RecurrentAttention
...
class Trainer:
    ...
    def __init__(self, config, data_loader):
        ...
        self.pgd_attack = PGD(config.batch_size, self.num_layers, self.hidden_size, self.device, hp= {
        "alpha": 0.01,
        "steps": 30,
        "epsilon": 0.22
    })
    ...
    def train_one_epoch(self, epoch):
        ...
        with tqdm(total=self.num_train) as pbar:
            for i, (x, y) in enumerate(self.train_loader):
                ...
                if self.is_attacked:
                    adversarial_batch_data = self.pgd_attack.perturb(self.model, x, y, h_t, l_t)
                    x = adversarial_batch_data
                ...
                # vals, indices = torch.mode(predicted, -1)
                vals = predicted[:,-1] # last one as prediction
                R = (vals == y).float()
                R = R.unsqueeze(1).repeat(1, predicted.shape[1])
                
                # get the classification mask
                mask = torch.ones(predicted.shape, dtype=torch.bool).to(self.device)
                idx = torch.transpose(y.repeat(predicted.shape[1], 1), 1, 0)==predicted   
                ...
                last = logits[:,-1,:]
                logp = torch.gather(last.reshape(-1, self.num_classes), 1, yidx)
                # loss_action = F.nll_loss(logits[:,-1,:].reshape(-1, self.num_classes), y.reshape(-1))
                loss_action = -(logp).mean() # -(logp*(mask.clone().view(-1, 1))).mean()
                ...
                acc = 100 * (correct.sum().float() / len(y))
    ...
    def validate(self, epoch):
        ...
        for i, (x, y) in enumerate(self.valid_loader):
            ...
            idx = torch.transpose(y.repeat(predicted.shape[1], 1), 1, 0)==predicted  #y==10   
            for midx in range(mask.shape[0]):
                maskindex = idx[midx, :].nonzero()
                if len(maskindex)>0:
                    mask[midx, maskindex[0]+1:] = False
