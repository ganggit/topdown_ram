...
class GlimpseCovNet(nn.Module):
    ...
    def __init__(self, h_g, h_l, g, k, s, c):
        ...
        # glimpse layer
        D_in = k * g * g * c
        self.retina = CNetwork(g, 2048, D_in)
        self.fc1 = nn.Linear(D_in, h_g)
...
class CoreRNN(nn.Module):
    ...
    def forward(self, g_t, ht_prev):
        h1 = self.i2h(g_t)
        output, ht = self.lstm(h1, ht_prev)

        return output, ht

class  ContextNetwork(nn.Module):
    def __init__(self, n_size, hidden_size):
        super().__init__()
...
class  CNetwork(nn.Module):
    def __init__(self, im_size, n_size, hidden_size, channel=3):
        super().__init__()
        self.im_size = im_size
        self.conv1 = nn.Conv2d(channel, 64, kernel_size=(5,5), stride=1)
        self.conv2 = nn.Conv2d(64, 64, kernel_size=(3,3), stride=1)
        self.conv3 = nn.Conv2d(64, 128, kernel_size = (3,3), stride=1)
        self.fc = nn.Linear(n_size, hidden_size)
...
class DQN(nn.Module):
    def __init__(self, input_dim, num_class=10, channel = 3, num_actions=4, temperature = 1, init_w = 1e-3):
        super(DQN, self).__init__()
        self.input_dim = input_dim
        self.hidden = num_class
        self.num_actions = num_actions
        self.counts = [0 for col in range(num_actions)]
        self.values = torch.zeros(num_actions)
        self.temperature = temperature
        self.nrows = 2
        self.fc = nn.Linear(int(self.input_dim*self.input_dim*channel/num_actions), self.hidden)
        self.output = nn.Linear(self.num_actions*self.hidden, num_actions)
        
        # self.fc.weight.data.uniform_(-init_w, init_w)
        # self.fc.bias.data.uniform_(-init_w, init_w)

    def forward(self, img):
        self.ncols = int(self.num_actions/self.nrows)
        interval_row = int(self.input_dim/self.nrows)
        interval_col = int(self.input_dim/self.ncols)
        feats = []
        for i in range(0, self.nrows):
            for j in range(0, self.ncols):
                phi = img[:,:, i*interval_row:(i+1)*interval_row, j*interval_col:(j+1)*interval_col]
                x = torch.reshape(phi, [phi.shape[0], -1])
                x = self.fc(x)
                x = F.relu(x)
                feats.append(x)
        features = torch.cat(feats, 1)	
        scores = self.output(features)
        return scores

    def sample(self, x, epsilon=0.1):
        values = self.forward(x)
        z = sum([math.exp(v / self.temperature) for v in values])
        probs = [math.exp(v / self.temperature) / z for v in values]
